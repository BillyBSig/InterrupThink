"""Run the page consult in this folder, using the page fixture."""
...
from config import BAD, CONTINUATION, CONTINUED, DEFAULT_SANDBOX, FIXTURES, PAGE_DOCUMENT, RUNS  # noqa: E402
from graph import PageRetrieve, build_retriever, run_live_page, run_page_consult  # noqa: E402
...
def read_fixtures() -> tuple[str, str, str]:
    ...
    page = (FIXTURES / "page.txt").read_text(encoding="utf-8").strip()
    return task, question, page


def analyze(result, sandbox: Path) -> dict:
    package_path = sandbox / "page-package.txt"
    first = result.first
    return {
...
def run_cookbook(*, named: bool, assistant, make_checker, sandbox: Path):
    """Run the assistant. The fixture page is what the retriever returns."""
    task, _question, page = read_fixtures()
    ...
    result = run_page_consult(monitor, assistant, make_checker, tool, task)
    sandbox.mkdir(parents=True, exist_ok=True)
    (sandbox / "page-package.txt").write_text(result.package, encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="LlamaIndex page consult cookbook")
