"""AutoGen: pass reviewed information between two agents.
...
Flow
    ...
    The host runs two ``run_session`` calls and forwards the reviewed text.
    AutoGen manages agent configuration; the thinking checks remain in the
    sessions. One ``LlmMonitor`` observes the handoff.
...
Usage (repo root; local venv)::
    ...
    python3 cases/autogen-pipe/run.py   # personal .env keys
"""
...
from graph import analyze, run_autogen_pipe, _write_xml  # noqa: E402

analyze = analyze
run_autogen_pipe = run_autogen_pipe
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="AutoGen two-agent handoff")
