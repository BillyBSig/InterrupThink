"""LangGraph: correct a wrong host claim, then write staging.

Scenario
    An existing LangGraph has an agent node and the official ``ToolNode``.
    The ticket says production, while the supervisor knows the target is
    staging. After the correction, the session rolls back and ToolNode may
    write staging.txt.

Flow
    The ``agent`` node runs ``run_session`` and the ``tools`` node remains
    ``langgraph.prebuilt.ToolNode``. The normal tool boundary is preserved
    after the corrected session resumes.
...
from graph import analyze, run_correct  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="LangGraph correct-then-continue")
    ...
    payload = analyze(run_correct(interrupt=True, sandbox=live_root))
