"""Two AutoGen agents, each run_session, one supervisor. Optional extra, not a package."""
...
def _case():
    ...
    return _load(CASE / "run.py", "autogen_pipe_run")
...
def test_case_is_two_session_agents_not_package_or_mesh(tmp_path: Path):
    pytest.importorskip("autogen")
    ...
    assert "run_autogen_pipe" not in public
    assert "FixtureRetrieveTool" not in public
    assert "**proposed**" in adr
    assert "Status: **accepted**" not in adr
    ex = _case()
    out = _wired(ex, interrupt_stale=True, sandbox=tmp_path)
    assert isinstance(out.retrieve_agent, ConversableAgent)
    assert isinstance(out.write_agent, ConversableAgent)
    assert out.retrieve_agent.human_input_mode == "NEVER"
    assert out.write_agent.human_input_mode == "NEVER"


def test_example_calls_interruptible_not_the_case():
    example = (REPO / "examples" / "autogen_two_specialists.py").read_text(encoding="utf-8")
    ...
    assert "run_autogen_pipe" not in example
